...
int main(int argc, char* argv[]) {
    ...
    string outputPath = "./outputs/HPF_OMP_outputRes.png";
...
#pragma omp parallel for collapse(2)
    ...
    // You can also specify threads directly in the pragma
#pragma omp parallel for collapse(2)
    ...
    // Save execution time and thread info
    ofstream timeFile(timePath);
    if (timeFile.is_open()) {
        timeFile << "Execution time: " << duration.count() << " ms" << endl;
        timeFile << "Number of threads: " << omp_get_max_threads() << endl;
